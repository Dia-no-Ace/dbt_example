...
23) добавим pre-commit файл в корень репо для проверки кода на локальной машине до того, как запушим.
24) активируем командой `pre-commit install`
25) теперь при создании коммита будут запускаться тесты, указанные в конфиге pre-commit (если не хотим прибегать к тестам, пишем `git commit -m --no-verify`). Внимание! Выпилен mirriw-prettier - пакет не устанавливался
26) создаем шаблон описания пул-реквеста в папке .github (можно копировать этот текст в поле коммента к PR)
27) перейдем к настройке CI
28)
